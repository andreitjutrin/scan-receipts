"""
Admin Lambda — backup management endpoints.

GET  /admin/backups         — list all backup files available to restore
POST /admin/restore-backup  — restore a named backup as master.xlsx
"""
...
def lambda_handler(event, context):
    ...
    try:
        if method == "GET" and "backups" in path:
            ...
            if success:
                return _ok({"restored": filename})
            else:
                return _error(500, f"Failed to restore {filename}")
