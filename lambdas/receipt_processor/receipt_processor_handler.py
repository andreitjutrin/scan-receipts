...
S3 Event → extract bucket/key → run Textract → hybrid matching → save results.

Also handles POST /receipts/{receipt_id}/reprocess for manual retrigger.
"""
...
def _all_keywords(store_id: str) -> list[str]:
...
# ---------------------------------------------------------------------------
# Hybrid matcher — 4 layers
# ---------------------------------------------------------------------------
def match_item(normalized: str, store_id: str, strip_prefixes: list):
    """Returns (category, source, confidence, trust, needs_review)"""

    # Layers 1 + 2: exact match in cache
    ...
    if best_partial == 100 and best_kw:
        ...
        if mapping:
            conf = 0.95
            needs_review = conf < THRESHOLD_SILENT
            return mapping["category"], MatchSource.FUZZY, conf, TrustLevel.CONFIDENT, needs_review
    ...
    if keywords:
        ...
        if result:
            ...
            if mapping:
                ...
                trust = (TrustLevel.CONFIDENT if conf >= THRESHOLD_SILENT
                         else TrustLevel.TENTATIVE)
                needs_review = conf < THRESHOLD_SILENT
                return mapping["category"], MatchSource.FUZZY, conf, trust, needs_review
...
def parse_textract(resp: dict):
    """Returns (date, total, header_text, line_items)"""
...
def process_receipt(receipt_id: str, bucket: str, s3_key: str, store_id: str = "unknown"):
    ...
    # Cost safeguard — check + increment process_count atomically
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            # Increment match count for exact hits
        ...
        # Save items
        db.delete_items(receipt_id)  # clear any previous items if reprocessing
...
# ---------------------------------------------------------------------------
# Lambda handler — handles both S3 events and API reprocess calls
# ---------------------------------------------------------------------------
def lambda_handler(event, context):

    # S3 event trigger (automatic after photo upload)
    if "Records" in event:
        for record in event["Records"]:
            if record.get("eventSource") == "aws:s3":
                bucket = record["s3"]["bucket"]["name"]
                s3_key = record["s3"]["object"]["key"]
                # Extract receipt_id from key: receipts/{receipt_id}/original.jpg
                parts      = s3_key.split("/")
                receipt_id = parts[1] if len(parts) >= 2 else None
                if receipt_id:
                    # Get store_id from the pending receipt record
